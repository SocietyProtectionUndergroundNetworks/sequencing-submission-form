...
# Rscript -e "rmarkdown::render(input='sequencing_report_opts.rmd', output_format='all',output_dir='test3')" \
...
# Save file. To open in R use: physeq_decontam <- readRDS("physeq_decontam.Rdata")
...
physeq_filtered <- prune_samples(
  sample_sums(physeq_decontam) >= 10,
  physeq_decontam)
...
```
```{r include=FALSE}
#ggsave(paste0(output_dir,"/","LibrarySize.pdf"))
```
