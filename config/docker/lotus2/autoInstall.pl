...
sub compile_LCA($){
	my ($ldi2) = @_;
  my $expPath = "/home/condauser/miniconda/envs/lotus2_env/bin/LCA";
...
sub compile_rtk($){
	my ($ldi2) = @_;
  my $expPath = "/home/condauser/miniconda/envs/lotus2_env/bin/rtk";
	if (-e $expPath){#test if can excecute locally
		my $sdmV = `$expPath -v`;
...
sub compile_sdm($){
	my ($ldi2) = @_;
	my $expPath = "/home/condauser/miniconda/envs/lotus2_env/bin/sdm";
	if (-e $expPath){#test if can excecute locally
		my $sdmV = `$expPath -v`;
...
sub user_options(){
  ...
  # Print the message indicating the selection
  print "Doing selection 2 for SILVA database installation...\n";

  # Proceed to set the installation flag for SILVA
  $refDBinstall[2] = 1;  # Set the SILVA database flag
  # Reset other flags if needed
  $refDBinstall[1] = 0;   # KSGP
  $refDBinstall[3] = 0;   # GreenGenes2
  $refDBinstall[4] = 0;   # HITdb
  $refDBinstall[5] = 0;   # PR2
  $refDBinstall[6] = 0;   # beeTax
  $refDBinstall[8] = 0;   # Combined database option
  ...
  if (!$skipAll && ($refDBinstall[2] || $refDBinstall[8])) {
      ...
      # Hardcoding the acceptance of the SILVA license
      my $user_input = "y";  # Simulating user input as 'yes'
      
      # Using the hardcoded response instead of waiting for user input
      chomp($user_input);  # Ensure it's clean input (not strictly necessary for hardcoded input)

      if ($user_input eq "y" || $user_input eq "Y" || $user_input eq "yes") {
